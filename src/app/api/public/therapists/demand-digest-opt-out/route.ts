...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (!res.ok || !res.therapistId) {
      void track({ type: 'demand_digest_optout_failed', level: 'warn', source: 'api.therapists.demand-digest-opt-out', props: { reason: res.reason } });
      return new NextResponse(html('<h1>Link abgelaufen oder ungültig</h1><p>Bitte öffne den neuesten Abmelde‑Link oder kontaktiere uns.</p>'), { status: 400, headers: { 'Content-Type': 'text/html; charset=utf-8' } });
    }
    ...
    const { data: t, error } = await supabaseServer
      ...
      .single();
    if (error || !t) {
      await logError('api.therapists.demand-digest-opt-out', error, { stage: 'fetch', therapist_id: tid }, ip, ua);
    ...
    const nextMeta = { ...base, notifications: { ...notifications, demand_digest_opt_out: true } };
    ...
    const { error: upErr } = await supabaseServer
      ...
      .eq('id', tid);
    if (upErr) {
      await logError('api.therapists.demand-digest-opt-out', upErr, { stage: 'update', therapist_id: tid }, ip, ua);
    ...
    void track({ type: 'demand_digest_opted_out', level: 'info', source: 'api.therapists.demand-digest-opt-out', props: { therapist_id: tid } });
    return new NextResponse(html('<h1>Abgemeldet</h1><p>Du erhältst keine monatlichen Nachfrage‑Übersichten mehr. Du kannst dich jederzeit über dein Therapeuten‑Portal wieder anmelden.</p>'), { status: 200, headers: { 'Content-Type': 'text/html; charset=utf-8' } });
  } catch (e) {
    await logError('api.therapists.demand-digest-opt-out', e, { stage: 'exception' });
