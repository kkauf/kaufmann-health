    ...
    <section className="space-y-6">
      ...
      {data?.funnels && (
        <div className="grid grid-cols-1 gap-4">
          {(() => {
            const steps = data.funnels?.quizMatches?.steps || [];
            const start = steps[0]?.count || 0;
            const sent = steps[steps.length - 1]?.count || 0;
            return (
              <Card>
                <CardHeader>
                  <CardTitle>Quiz Matches Funnel</CardTitle>
                  <CardDescription>Golden Metric: Messages Sent (Matches): {sent}</CardDescription>
                </CardHeader>
                <CardContent>
                  {!steps.length ? (
                    <div className="text-sm text-muted-foreground">Keine Daten</div>
                    ...
                    <div className="space-y-2">
                      {steps.map((s, i) => {
                        ...
                        return (
                          <div key={`quiz-${s.name}-${i}`} className="flex items-center gap-2 text-xs">
          ...
          {(() => {
            const steps = data.funnels?.browseDirectory?.steps || [];
            const start = steps[0]?.count || 0;
            const sent = steps[steps.length - 1]?.count || 0;
            return (
              <Card>
                <CardHeader>
                  <CardTitle>Directory Browse Funnel</CardTitle>
                  <CardDescription>Golden Metric: Messages Sent (Directory): {sent}</CardDescription>
                </CardHeader>
                <CardContent>
                  {!steps.length ? (
                    <div className="text-sm text-muted-foreground">Keine Daten</div>
                    ...
                    <div className="space-y-2">
                      {steps.map((s, i) => {
                        ...
                        const fromStart = s.from_start_rate;
                        return (
                          <div key={`dir-${s.name}-${i}`} className="flex items-center gap-2 text-xs">
                            <div className="w-48 text-right text-muted-foreground truncate">{s.name}</div>
                            <div className="flex-1 bg-gray-100 rounded-sm h-6 relative overflow-hidden">
