...
export function renderPatientMatchFound(params: {
  ...
  lines.push('<p style="margin:0 0 12px;">gute Neuigkeiten! Wir haben eine:n passende:n Therapeut:in für Sie gefunden.</p>');
  lines.push('<div style="background-color:#F9FAFB; padding:16px; border-radius:8px; border:1px solid #E5E7EB; margin: 12px 0;">');
  lines.push(`<p style=\"margin:0; font-weight:600; color:#1A365D;\">${therapist ? escapeHtml(therapist) : 'Ihr:e Therapeut:in'}</p>`);
  if (specs.length > 0) {
    lines.push(`<p style=\"margin:4px 0 0; color:#374151;\">Spezialisierung: ${escapeHtml(specs.join(', '))}</p>`);
  }
  lines.push('</div>');
  lines.push('<p style="margin:0 0 12px;">Der/die Therapeut:in wird Sie in der Regel innerhalb von 24&nbsp;Stunden kontaktieren. Falls Sie in dieser Zeit nichts hören, antworten Sie gerne auf diese E‑Mail, dann kümmern wir uns sofort darum.</p>');
  ...
  const html = renderLayout({
    ...
    contentHtml: [
      `<h1 style=\"color:#1A365D; font-size:22px; margin:0 0 12px;\">Ihr:e Therapeut:in wird sich melden</h1>`,
      ...lines,
    ].join(''),
  });

  return {
    subject: `Ihr:e Therapeut:in wird sich melden${therapist ? ` - ${therapist}` : ''}`,
    html,
  };
}
...
export function renderPatientCustomUpdate(params: {
  ...
  const contentHtml = `
    <h1 style=\"color:#1A365D; font-size:22px; margin:0 0 12px;\">Update zu Ihrer Therapeut:innensuche</h1>
  ...
  return {
    subject: 'Update zu Ihrer Therapeut:innensuche',
