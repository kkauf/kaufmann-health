...
export function renderCalSessionFollowup(params: CalSessionFollowupParams): EmailContent {
  ...
  if (hasNextSlot) {
    ...
    lines.push(`<p style="margin:0; font-size:15px; line-height:1.65; color:#065f46 !important;">Regelmäßige Sitzungen können Ihren Fortschritt unterstützen.</p>`);
  }
  lines.push('</div>');
  ...
  lines.push('<p style="margin:0; color:#64748b !important; font-size:14px; line-height:1.6;">Kein Problem – Sie entscheiden über Ihr Tempo. Wir sind da, wenn Sie bereit sind.</p>');
  lines.push('</div>');
