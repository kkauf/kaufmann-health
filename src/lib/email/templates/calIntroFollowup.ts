...
export function renderCalIntroFollowup(params: CalIntroFollowupParams): EmailContent {
  ...
  const lines: string[] = [];
  lines.push('<h1 style="color:#0f172a !important; font-size:28px; font-weight:700; margin:0 0 16px; line-height:1.3; letter-spacing:-0.02em;">Wie war dein Kennenlerngespräch?</h1>');
  lines.push(`<p style="margin:0 0 16px; font-size:16px; line-height:1.65; color:#475569 !important;">Hallo${name ? ` ${esc(name)}` : ''},</p>`);
  lines.push(`<p style="margin:0 0 16px; font-size:16px; line-height:1.65; color:#475569 !important;">Du hattest gerade ein Kennenlerngespräch mit <strong>${therapist}</strong>. Wir hoffen, es war ein guter erster Kontakt!</p>`);
  ...
  lines.push('<p style="margin:0 0 12px; font-size:16px; line-height:1.65; color:#064e3b !important;"><strong>Möchtest du eine Therapiesitzung buchen?</strong></p>');
  ...
  if (hasNextSlot) {
    ...
    lines.push(`<p style="margin:0; font-size:15px; line-height:1.65; color:#065f46 !important;">Wenn ${therapist} gut zu dir passt, kannst du direkt eine Therapiesitzung buchen.</p>`);
  }
  lines.push('</div>');
  ...
  lines.push('<p style="margin:0 0 12px; color:#64748b !important; font-size:14px; line-height:1.6;">Das ist völlig in Ordnung – die Chemie muss stimmen. Du kannst dir gerne andere Therapeut:innen anschauen oder uns kontaktieren.</p>');
  ...
  const subject = `Wie war dein Kennenlerngespräch mit ${params.therapistName}?`;
  const html = renderLayout({
    title: 'Wie war dein Kennenlerngespräch?',
    ...
    preheader: hasNextSlot
      ...
      : 'Möchtest du einen Folgetermin buchen?',
  });
