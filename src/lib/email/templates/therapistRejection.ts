...
export function renderTherapistRejection(params: {
  ...
  const lines: string[] = [];
  lines.push('<h1 style="color:#0f172a !important; font-size:28px; font-weight:700; margin:0 0 16px; line-height:1.3; letter-spacing:-0.02em;">Rückfrage zu deinem Profil</h1>');
  lines.push(`<p style=\"margin:0 0 16px; font-size:16px; line-height:1.65; color:#475569 !important;\">Hi${name ? ` ${escapeHtml(name)}` : ''},</p>`);
  lines.push('<p style="margin:0 0 20px; font-size:16px; line-height:1.65; color:#475569 !important;">Vielen Dank für dein Interesse. Wir benötigen noch Anpassungen an deinem Profil:</p>');

  lines.push('<div style="background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%) !important; background-image: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%) !important; padding:20px 24px; border-radius:12px; border:1px solid rgba(251, 191, 36, 0.3); margin: 16px 0 20px; box-shadow: 0 2px 4px 0 rgba(251, 191, 36, 0.1);">');
  lines.push('<ul style="margin:0 0 0 20px; color:#78350f !important; font-size:15px; line-height:1.65;">');
  ...
  if (params.uploadUrl) {
    lines.push(`<p style=\"margin:0 0 20px; font-size:16px; line-height:1.65; color:#475569 !important;\">Bitte aktualisiere dein Profil: <a href=\"${params.uploadUrl}\" style=\"color:#10b981 !important; text-decoration:none; font-weight:700;\">Profil vervollständigen →</a></p>`);
  }
  ...
  lines.push('<p style="color:#64748b !important; font-size:14px; margin:0; line-height:1.6;">Bei Fragen antworte gerne auf diese E‑Mail.</p>');
  lines.push('</div>');
